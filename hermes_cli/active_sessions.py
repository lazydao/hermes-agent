...
def _read_entries(path: Path, *, strict: bool = False) -> list[dict[str, Any]]:
    ...
    for entry in valid:
        ...
        if process_start_token is not None and (
            _optional_process_start_token(process_start_token) is None
        ):
            raise ActiveSessionRegistryError(
                f"active session registry contains an invalid process start token: {path}"
            )
    return valid
...
def _pid_liveness(pid: Any, process_start_token: Any = None) -> Optional[bool]:
...
def _prune_dead(
    ...
    for entry in entries:
        ...
        if strict or tracked:
            state = _pid_liveness(entry.get("pid"), entry.get("process_start_token"))
            if state is None:
                raise ActiveSessionRegistryError(
